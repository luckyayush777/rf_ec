...
- Expand **Cleaning** in the equipment panel for per-part progress and **Compare before**. At 90% overall cleanliness, refit the parts and reconnect the cable to **Finish service**. **Next card** resets dust once everything is assembled.

This is a feedback prototype: jobs and cleaning progress live in memory, cards reuse the same fictional model, and repasting is not implemented. Fine dust uses small per-part surface masks, buildup uses removable clumps, and the blower has a bounded particle pool and synthesized air sound. Front and back masks are independent.
